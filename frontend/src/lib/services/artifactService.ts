...
export async function createZoningSnapshotArtifact(input: {
  ...
  const district = await findZoningDistrictByPoint({ city: input.city, lat: geocode.lat, lng: geocode.lng });

  if (poolConfigured && !district) {
    throw new Error("No zoning district found for this location (load zoning_districts first).");
  }

  const rules = district
    ? await getZoningRulesForZone({ city: input.city, zone_code: district.zone_code })
    : null;
